...
contract EscrowReuseTest is Test {
    ...
    function testEscrowIdCannotBeReusedAfterSettle() external {
        ...
        vm.prank(alice);
        bytes32 tx1 = kernel.createTransaction(bob, alice, ONE_USDC, block.timestamp + 7 days, 2 days, keccak256("service1"), 0);
        ...
        vm.prank(attacker);
        bytes32 tx2 = kernel.createTransaction(bob, attacker, ONE_USDC, block.timestamp + 7 days, 2 days, keccak256("service2"), 0);
    ...
    function testEscrowIdCannotBeReusedAfterCancel() external {
        ...
        vm.prank(alice);
        bytes32 tx1 = kernel.createTransaction(bob, alice, ONE_USDC, block.timestamp + 1 days, 2 days, keccak256("service"), 0);
    ...
    function testActiveTransactionsCannotShareEscrowId() external {
        ...
        vm.prank(alice);
        bytes32 tx2 = kernel.createTransaction(bob, alice, ONE_USDC, block.timestamp + 7 days, 2 days, keccak256("service2"), 0);
    ...
    function testFuzzEscrowIdNotReusableAfterCompletion(bytes32 escrowId) external {
        ...
        vm.prank(alice);
        bytes32 tx1 = kernel.createTransaction(bob, alice, ONE_USDC, block.timestamp + 7 days, 2 days, keccak256("service"), 0);
